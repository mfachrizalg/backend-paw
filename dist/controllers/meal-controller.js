import { MealService } from "../services/meal-service.js";
import { translate } from "../utils/translate.js";
export class MealController {
    static async getMeal(req, res, next) {
        try {
            const name = req.query.name;
            const translatedName = await translate(name);
            const response = await MealService.getMeal(translatedName);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
    static async addMealToSchedule(req, res, next) {
        try {
            const response = await MealService.addMealToSchedule(req.body, req);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
    static async bookmarkMeal(req, res, next) {
        try {
            const mealDBid = req.params["mealDBid"];
            const response = await MealService.bookmarkMeal(mealDBid, req);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
    static async getBookmarkedMeals(req, res, next) {
        try {
            const response = await MealService.getBookmarkedMeals(req);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
    static async getScheduleMeals(req, res, next) {
        try {
            const response = await MealService.getScheduleMeals(req);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
    static async deleteBookmark(req, res, next) {
        try {
            const response = await MealService.deleteBookmark(req.params["mealId"], req);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
    static async deleteSchedule(req, res, next) {
        try {
            const response = await MealService.deleteSchedule(req.params["mealId"], req);
            res.status(200).json(response);
        }
        catch (error) {
            next(error);
        }
    }
}
//# sourceMappingURL=meal-controller.js.map